...
float TsImageCore::GetPixelTex2D(int x, int y, int reso ) 
{
	int channel = 0;///////
	int w = mTex->GetSizeX();
	int h = mTex->GetSizeY();
	...
	uint8* data = (uint8*)mImage + (px + py * w) * byte_per_pixel ;
...
float	TsImageCore::GetPixelTex2D_Catmull(float x, float y, int reso )
{
	...
	auto get_pixel = [&] ( int _x, int _y )->float{
		return TsImageCore::GetPixelTex2D( _x, _y, reso ) ;
	};
	int   dx  = (int)x ;
	int   dy  = (int)y ;
...
float	TsImageCore::GetPixelTex2D_Linear(float x, float y, int reso )
{
	...
	float h00 = TsImageCore::GetPixelTex2D( dx+0, dy+0, reso ) ;
	float h01 = TsImageCore::GetPixelTex2D( dx+1, dy+0, reso ) ;
	float h10 = TsImageCore::GetPixelTex2D( dx+0, dy+1, reso ) ;
	float h11 = TsImageCore::GetPixelTex2D( dx+1, dy+1, reso ) ;
